...
census_table <- read_csv("../Eric/ces2019_web_all_predictors.csv") %>% 
  mutate_if(is.character, as.factor) %>%
  select(-income_number, -cps19_ResponseId)

census_sum_1 <- summary(census_table[1:6])
census_sum_2 <- summary(census_table[7:12])
census_sum_3 <- summary(census_table[13:18])

census_clean_1 <- as.data.frame(apply(census_sum_1, 2, function(x) clean_colon(x)))

census_clean_2 <- as.data.frame(apply(census_sum_2, 2, function(x) clean_colon(x)))

census_clean_3 <- as.data.frame(apply(census_sum_2, 2, function(x) clean_colon(x)))

knitr::kable(tibble(census_clean_1), align = "cccccccccccccccccc", caption = "CES 2019 Summary Statistics")
knitr::kable(tibble(census_clean_2), align = "cccccccccccccccccc", caption = "CES 2019 Summary Statistics")
knitr::kable(tibble(census_clean_3), align = "cccccccccccccccccc", caption = "CES 2019 Summary Statistics")
```
...
survey_table <- read_csv("../Eric/gss_clean_poststrat.csv") %>% 
  mutate_if(is.character, as.factor)
summarised_1 <- summary(survey_table[1:5])
...
summarised_2 <- summary(survey_table[6:11])
...
```{r, include=FALSE}
survey_data <- read_csv("../Eric/gss_clean_poststrat.csv")
census_data <- read_csv("../Eric/ces2019_web_all_predictors.csv")
```
...
```
Figure 1 tells us some information about our survey data. We can see that there were more participants in the survey data with income levels of "25,000 to 49,999" and "Less than 25,000" than the rest of the categories. In the cleaned census data, there is less spread in the income categories, and looks more representative for each category. This tells us a about how the survey data could have bias' towards certain income groups, which is why we will post-stratify to match the census population.

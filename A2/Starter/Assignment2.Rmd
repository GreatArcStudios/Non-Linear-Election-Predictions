...
From this data, we can see that $\hat p_{old}$ = 0.374, and $\hat p_{young}$ = 0.319. The test statistic, $$Z = \frac{\hat p_{old}- \hat p_{young}}{\hat p (1 - \hat p) (\frac{1}{n_{old}} + \frac{1}{n_{young}})}$$ 
will determine the p-value which will tell us whether or not we should reject the null hypothesis. A built in function called $prop.test()$ was used. This function takes in two vectors, where each vector contains the groups proportion and number of observations.

After conducting a two proportion Z test at a 95% confidence level, there was strong evidence against the null hypotheses ($p$ =`r as.numeric(test$p.value)`). We can interpret that there is a difference between the proportion of liberal votes for people aged 15-24 and the proportion of liberal votes for people aged 65-74. This gives further evidence that age is a good predictor for voting outcomes.

...
ces2019_web_cleaned_no_na <- ces2019_web_cleaned %>% drop_na()

write_csv(ces2019_web_cleaned %>% drop_na(), "ces2019_web_cleaned_no_na.csv")
...
gss_poststrat_weights <- gss_clean %>% 
  mutate(total_samples = n()) %>% 
  group_by(across(c(province, language, income_category, age_category, marital_status, education))) %>% 
  mutate(category_samples = n()) %>% 
  mutate(poststrat_weight = n()/total_samples) %>% 
  select(c(total_samples, category_samples, poststrat_weight)) %>% 
  unique()

write_csv(gss_poststrat_weights, "gss_poststrat_weights.csv")

...
save(rfe_predictors, "rfe_predictors.RData")
```
...
ran_effects.reduced.predictions
```
...
colnames(fixed_effects.full.predictions) <- c("fixed_effects.lib_prob",  "fixed_effects.con_prob", "fied_effects.ndp_prob", "fixed_effects.blc_prob", "fixed_effects.grn_prob", "fixed_effects.ppl_prob")
...
colnames(fixed_effects.reduced.predictions) <- c("fixed_effects.lib_prob",  "fixed_effects.con_prob", "fied_effects.ndp_prob", "fixed_effects.blc_prob", "fixed_effects.grn_prob", "fixed_effects.ppl_prob")
...
```{r}
test_full <- test_full %>% mutate(province = fct_recode(province, "Manitoba" = "Maintoba")) %>% mutate_if(is.factor, as.character)
test_reduced <- test_reduced %>% mutate(province = fct_recode(province, "Manitoba" = "Maintoba")) %>% mutate_if(is.factor, as.character)
# load popluation datasets 
...
poststrat.reduced.combined_national_df <- census_reduced %>% 
  right_join(reduced_predictions_df) %>% drop_na() %>% mutate_if(is.character, as.factor) %>% 

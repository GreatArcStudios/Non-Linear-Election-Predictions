...
```
```{r, echo = TRUE}
plot.tradGPUCores <- sim.survey %>% ggplot(aes(x = weeklyHours, y = simulations.tradGPUCores)) + geom_point(aes(color = user)) + labs(title = "Weekly Hours of GPU Usage versus Traditional GPU Cores Preference Score", x = "Weekly Hours of GPU Usage", y = "Traditional GPU Cores Preference Score") + theme_minimal()

plot.tradGPUCores <- sim.survey %>% ggplot(aes(x = weeklyHours, y = simulations.tradGPUCores)) + geom_point(aes(color = user)) + labs(title = "Weekly Hours of GPU Usage versus Traditional GPU Cores Preference Score", x = "Weekly Hours of GPU Usage", y = "Traditional GPU Cores Preference Score")
...
Our analysis will consist of mixed effects models that separately measure the preference for traditional GPU cores, ray-tracing, and half-precision matrix math hardware. In particular, we will have three models using the same model specification but different response variables. So we will have a model for each one of the preferences for traditional GPU cores, ray-tracing, and half-precision matrix math hardware. We will construct these three models separately because we want to be able to examine each response variable, i.e., the preferences of the three categories of hardware separately. This can allow us to see if we get different effect size estimates for the same covariates with a different response variable. We'll specifically model the response variable using a generalized linear mixed effects model, where the response is given as a normal distribution. We will define the model as follows: 

$$
\begin{aligned}
y_i &\sim N(\mu_i, \sigma^2) \\
\mu_i = \beta_0 + (\beta_1 + U_{1j})\cdot \text{Primary GPU Interaction} &+ (\beta_2 + U_{2j})\cdot\text{Type of GPU}_i + \beta_2 \cdot \text{Estimated Hours of Daily GPU Usage}_i + U_{3k} + \epsilon_i \\
U_{1} &\sim N(0, \tau_1) \\
U_2 &\sim N(0, \tau_2) \\
U_{3} &\sim N(0, \tau_{3})
\end{aligned}
...
```{r, include = FALSE}
model.tradGPUCores <- lmer(simulations.tradGPUCores ~ weeklyHours, data = sim.survey, family = normal())

#  GPUType | GPUType  + primaryInteraction | primaryInteraction
```

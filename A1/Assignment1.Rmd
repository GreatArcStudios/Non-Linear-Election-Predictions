...
```
```{r, echo = TRUE}
plot.tradGPUCores <- sim.survey %>% ggplot(aes(x = weeklyHours, y = simulations.tradGPUCores)) + geom_point(aes(color = as.factor(user))) + labs(title = "Weekly Hours of GPU Usage versus Traditional GPU Cores Preference Score", x = "Weekly Hours of GPU Usage", y = "Traditional GPU Cores Preference Score") + theme_minimal()

plot.rayTraced <- sim.survey %>% ggplot(aes(x = weeklyHours, y = simulations.rayTraced)) + geom_point(aes(color = as.factor(user))) + labs(title = "Weekly Hours of GPU Usage versus Traditional GPU Cores Preference Score", x = "Weekly Hours of GPU Usage", y = "Traditional GPU Cores Preference Score")


plot.halfPrecision <- sim.survey %>% ggplot(aes(x = weeklyHours, y = simulations.halfPrecision)) + geom_point(aes(color = as.factor(user))) + labs(title = "Weekly Hours of GPU Usage versus Traditional GPU Cores Preference Score", x = "Weekly Hours of GPU Usage", y = "Traditional GPU Cores Preference Score")
...
The data was simulated specifically using the R language and the library `extraDistr` for sampling from a categorical distribution. 
...
So if we have a small p-value from the resulting test, then we have evidence that the more complex model is better. We will opt to not show the derivations of the test statistic as it is rather complex and out of the scope of this paper. 
...
qq_plot.pro.rayTraced <- pro.rayTraced %>% ggplot(aes(sample = simulations.rayTraced)) + stat_qq() + stat_qq_line()
qq_plot.con.rayTraced <- con.rayTraced %>% ggplot(aes(sample = simulations.rayTraced)) + stat_qq() + stat_qq_line()

qq_plot.pro.tradGPUCores <- pro.tradGPUCores %>% ggplot(aes(sample = simulations.tradGPUCores)) + stat_qq() + stat_qq_line()
qq_plot.con.tradGPUCores <- con.tradGPUCores %>% ggplot(aes(sample = simulations.tradGPUCores)) + stat_qq() + stat_qq_line()

qq_plot.pro.halfPrecision <- pro.halfPrecision %>% ggplot(aes(sample = simulations.halfPrecision)) + stat_qq() + stat_qq_line()
qq_plot.con.halfPrecision <- con.halfPrecision %>% ggplot(aes(sample = simulations.halfPrecision)) + stat_qq() + stat_qq_line()

qq_plot.con.tradGPUCores
qq_plot.pro.tradGPUCores

qq_plot.pro.rayTraced
qq_plot.con.rayTraced 

qq_plot.pro.halfPrecision
qq_plot.con.halfPrecision
```

```{r}
set.seed(123)

var.test(sim.survey %>% filter(user == 0) %>% select(simulations.rayTraced) %>% pull() %>% sample(1000), sim.survey %>% filter(user == 1) %>% select(simulations.rayTraced) %>% pull() %>% sample(1000))
```

...
For reference we fit the following models using the R package `lmer`: 

- Traditional GPU Cores Preference Scores: 
  - Reduced model: `r model.tradGPUCores <- lmer(simulations.tradGPUCores ~ weeklyHours + primaryInteraction + GPUType + (1 + primaryInteraction + GPUType | user), data = sim.survey)` 
  - Complex model:`r `model.extra.tradGPUCores <- lmer(simulations.tradGPUCores ~ weeklyHours + primaryInteraction + GPUType + (1 + primaryInteraction + GPUType + weeklyHours | user), data = sim.survey)`
- Ray Tracking Hardware Preference Scores: 
  - Reduced model: `r model.rayTraced <- lmer(simulations.rayTraced ~ weeklyHours + primaryInteraction + GPUType + (1 + primaryInteraction + GPUType | user), data = sim.survey)`
  - Complex model: `r model.extra.rayTraced <- lmer(simulations.rayTraced ~ weeklyHours + primaryInteraction + GPUType + (1 + primaryInteraction + GPUType + weeklyHours | user), data = sim.survey)` 
- Half Precision Matrix Math Hardware Preference Scores: 
  - Reduced model: `r `model.halfPrecision <- lmer(simulations.halfPrecision ~ weeklyHours + primaryInteraction + GPUType + (1 + primaryInteraction + GPUType | user), data = sim.survey)` 
  - Complex model: `r model.extra.halfPrecision <- lmer(simulations.halfPrecision ~ weeklyHours + primaryInteraction + GPUType + (1 + primaryInteraction + GPUType + weeklyHours | user), data = sim.survey)` 

```{r}
set.seed(123)
knitr::kable(lmtest::lrtest(model.extra.tradGPUCores, model.tradGPUCores))
```

The table above is the result of the likelihood ratio test between the complex model and the reduced model for the traditional GPU cores preference scores. As shown, the p-value is 0.632, so we have no evidence that the complex model for the traditional GPU cores preference scores being better. For conducting the likelihood ratio test, we use the following R function call `r lmtest::lrtest(model.extra.tradGPUCores, model.tradGPUCores)`.

```{r}
set.seed(123)
knitr::kable(lmtest::lrtest(model.extra.halfPrecision, model.halfPrecision))
```

The table above is the result of the likelihood ratio test between the complex model and the reduced model for the traditional GPU cores preference scores. As shown, the p-value is 0.12, so we have very little evidence that the complex model for the traditional GPU cores preference scores being better, which is insufficient to conclude that the complex model is necessary even at the 0.1 significance level. For conducting the likelihood ratio test, we use the following R function call `r lmtest::lrtest(model.extra.halfPrecision, model.halfPrecision)`.


```{r}
set.seed(123)
knitr::kable(lmtest::lrtest(model.extra.rayTraced, model.rayTraced))
```

The table above is the result of the likelihood ratio test between the complex model and the reduced model for the traditional GPU cores preference scores. As shown, the p-value is 0.978, so we have essentially no evidence that the complex model for the traditional GPU cores preference scores being better. For conducting the likelihood ratio test, we use the following R function call `r lmtest::lrtest(model.extra.rayTraced, model.rayTraced)`.
